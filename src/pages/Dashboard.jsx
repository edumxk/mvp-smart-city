...
// Ícones para o minimapa, diferenciando por status
const markerIconOpen = new L.Icon({
  iconUrl: "https://cdn-icons-png.flaticon.com/512/684/684908.png",
  iconSize: [32, 32],
  iconAnchor: [16, 32],
});

const markerIconResolved = new L.Icon({
...
const calculatePriorityScore = (report) => {
  if (report.status === 'Resolvido') return 0;
  let score = 0;
...
const StatCard = ({ icon, title, value, color, isActive, onClick }) => (
  <button onClick={onClick} className={`bg-white p-4 rounded-xl shadow-sm flex items-center gap-4 text-left w-full transition-all duration-200 ${color} ${isActive ? 'ring-2 ring-blue-500 shadow-md' : 'ring-0 ring-transparent hover:shadow-md'}`}>
...
const StatusTag = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${isResolved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
      {isResolved ? 'Finalizada' : 'Aberta'}
    </span>
  );
};
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 space-y-3">
          <div className="h-40 w-full rounded-lg overflow-hidden">
            <MapContainer center={[report.location.lat, report.location.lng]} zoom={15} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
              ...
              <Marker position={[report.location.lat, report.location.lng]} icon={report.status === 'Resolvido' ? markerIconResolved : markerIconOpen}></Marker>
            </MapContainer>
          </div>
          {report.status === 'Recebido' && (
...
export default function Dashboard() {
  ...
  const [activeFilter, setActiveFilter] = useState('Abertas');
  ...
  const { filteredReports, totalOpen, totalResolved, averageLikes } = useMemo(() => {
    ...
    let listToFilter = [];
    if (activeFilter === 'Abertas') listToFilter = openReports;
    else if (activeFilter === 'Resolvidas') listToFilter = resolvedReports;
    else listToFilter = reports;

    const reportsWithScore = listToFilter.map(report => ({ ...report, priorityScore: calculatePriorityScore(report) })).sort((a, b) => b.priorityScore - a.priorityScore);
    const avgLikes = reports.length > 0 ? (reports.reduce((sum, r) => sum + r.likes, 0) / reports.length).toFixed(1) : 0;
    return { filteredReports: reportsWithScore, totalOpen: openReports.length, totalResolved: resolvedReports.length, averageLikes: avgLikes };
  }, [reports, activeFilter]);

  const handleSelectReport = (id) => { setSelectedReportId(prevId => (prevId === id ? null : id)); };
  ...
  return (
    <div className="p-4 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-md">
        <h2 className="text-xl font-bold text-gray-700 mb-3">Lista de Ocorrências ({activeFilter})</h2>
